          ...
          Container(
            ...
            child: Consumer<ProductDetailsController>(
              builder: (ctx, productDetailsController, child) {
                ...
                final matchedItem = cartController.cartList.where((cartItem) => cartItem.productId == product.id).firstOrNull;

                bool isExistInCart = false;

                if ((product.choiceOptions?.isNotEmpty ?? false) || (product.digitalProductFileTypes?.isNotEmpty ?? false)) {
                  for (var cartItem in cartController.cartList) {
                    if (cartItem.productId == product.id && (cartItem.variant == variationType || cartItem.variant == variantKey)) {
                      isExistInCart = true;
                      break;
                    }
                  }
                } else {
                  isExistInCart = cartController.cartList.any((cartItem) {
                    return cartItem.productId == product.id;
                  });
                }

                int cartQuantity = matchedItem?.quantity ?? 0;

                if(isExistInCart && isFirstTime) {
                  WidgetsBinding.instance.addPostFrameCallback((_) {
                    Provider.of<ProductDetailsController>(context, listen: false).setQuantity(cartQuantity);
                  });
                  isFirstTime = false;
                }
                                          ...
                                          Expanded(
                                            child:  Wrap(
                                              ...
                                              children: List.generate(choice.options!.length, (i) {
                                                ...
                                                return InkWell(
                                                  onTap: () {
                                                    productDetailsController.setCartVariationIndex(1, index, i, context);
                                                    isFirstTime = true;
                                                  },
                                                  child: Container(
                                ...
                                itemBuilder: (ctx, index) {
                                  return Column(
                                    children: [
                                        ...
                                        children: [
                                          ...
                                          Expanded(
                                            child: Wrap(spacing: 8, runSpacing: 8,
                                              children: List.generate(extensions[index].length, (i) {
                                                final isSelected = productDetailsController.digitalVariationIndex == index && productDetailsController.digitalVariationSubindex == i;
                                                return InkWell(
                                                  onTap: () {
                                                    productDetailsController.setDigitalVariationIndex(widget.product!.minimumOrderQty, index, i, context);
                                                    isFirstTime = true;
                                                  },
                                                  child: Container(
                                                    padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                                                    decoration: BoxDecoration(
                                                      borderRadius: BorderRadius.circular(50),
                                                      color: isSelected ? Theme.of(context).primaryColor.withValues(alpha: 0.1) : Theme.of(context).hintColor.withAlpha(30),
                                                      border: Border.all(width: 1, color: isSelected ? Theme.of(context).primaryColor : Colors.transparent),
                                                    ),
                                                    child: Text(
                                                      extensions[index][i].trim(),
                                                      maxLines: 1,
                                                      overflow: TextOverflow.ellipsis,
                                                      style: titleRegular.copyWith(
                                                        fontSize: Dimensions.fontSizeDefault,
                                                        color: isSelected ? Theme.of(context).primaryColor : Theme.of(context).textTheme.titleMedium?.color,
                                                        fontWeight: isSelected ? FontWeight.w600 : FontWeight.w500,
                                                      ),
                                                    ),
                                                  ),
                                      ...
                                      if((variationFileType.length - 1) > index)
                    ...
                    child: Row(children: [
                      ...
                      SizedBox(
                        ...
                        child: TextField(
                          ...
                          onChanged: (value) {
                            final int? inputQty = int.tryParse(value);
                            final int min = widget.product!.minimumOrderQty ?? 1;
                            final bool isDigital = widget.product!.productType == "digital";

                            if (inputQty == null || inputQty < 1) return;

                            if (inputQty < min) {
                              final String current = productDetailsController.quantity.toString();
                              productQuantityController.text = current;
                              productQuantityController.selection = TextSelection.fromPosition(
                                TextPosition(offset: current.length),
                              );
                            } else if (!isDigital && inputQty > stock!) {
                              showOverlaySnackBar(context, getTranslated('out_of_stock', context), snackBarType: SnackBarType.warning);
                              final String fallback = min.toString();
                              productQuantityController.text = fallback;
                              productQuantityController.selection = TextSelection.fromPosition(
                                TextPosition(offset: fallback.length),
                              );
                              productDetailsController.setQuantity(min);
                            } else {
                              productDetailsController.setQuantity(inputQty);
                            }
                          },
                        ),
                      ),
